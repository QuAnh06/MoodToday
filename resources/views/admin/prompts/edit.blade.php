...
@section('title', 'Sửa Prompt')

@section('content')
<div class="content-header">
    <h1>Sửa Prompt</h1>
    <p>Chỉnh sửa nội dung prompt #{{ $prompt->id }}</p>
</div>
...
<div class="card">
    <form action="{{ route('admin.prompts.update', $prompt) }}" method="POST">
        ...
        <div class="form-group">
            <label for="type">Loại Prompt</label>
            <input type="text" id="type" name="prompt_type" class="form-control" value="{{ old('prompt_type', $prompt->prompt_type) }}" required placeholder="Ví dụ: general_advice, positive_affirmation...">
        ...
        <div class="form-group">
            ...
            <label for="mood_id">Mood liên quan (Tùy chọn)</label>
            <select name="mood_id" id="mood_id" class="form-control @error('mood_id') is-invalid @enderror">
                <option value="">Không có</option>
                @foreach($moods as $mood)
                    <option value="{{ $mood->id }}" {{ old('mood_id', $prompt->mood_id) == $mood->id ? 'selected' : '' }}>{!! $mood->emoji !!} {{ $mood->name }}</option>
                @endforeach
            </select>
            @error('mood_id') <p style="color: #c53030; font-size: 0.875rem; margin-top: 0.25rem;">{{ $message }}</p> @enderror --}}
            @error('mood_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>

        <div class="form-group">
            <label for="content">Nội dung Prompt</label>
            <textarea name="content" id="content" rows="8" class="form-control" required>{{ old('content', $prompt->content) }}</textarea>
            @error('content') <p style="color: #c53030; font-size: 0.875rem; margin-top: 0.25rem;">{{ $message }}</p> @enderror
        </div>

        <div class="form-group">
            <label for="version">Version</label>
            <input type="number" id="version" name="version" class="form-control" value="{{ old('version', $prompt->version) }}" required min="1">
